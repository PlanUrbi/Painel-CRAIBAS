...
(function () {
  ...
  const CATEGORIES = {
    ...
    educacao:    { label: 'Educação',      color: '#3498db', icon: 'fa-school',          class: 'cat-educacao' },
    assistencia: { label: 'Assistência',   color: '#9b59b6', icon: 'fa-hands-holding',   class: 'cat-assistencia' },
    seguranca:   { label: 'Segurança',     color: '#34495e', icon: 'fa-shield-halved',   class: 'cat-seguranca' },
  ...
  function renderSummaryStats() {
    ...
    const html = `
      <button class="summary-stat ${state.filters.categoria === 'todas' ? 'active' : ''}"
              data-cat="todas">
        <div class="summary-stat-icon" style="background:var(--primary);"><i class="fas fa-th"></i></div>
        <div class="summary-stat-num" data-anim>${EQUIPAMENTOS.length}</div>
        <div class="summary-stat-label">Total geral</div>
      </button>
      ${cats.map(catKey => {
